...
public class SchemaRegistryApi {
    ...
    @POST
    @Path("/validate")
    public Response validateSchema(tech.kayys.wayang.control.dto.SchemaValidationRequest request) {
        LOG.debug("Validating data against provided schema");

        return schemaRegistryService.validateSchema(request.schema(), request.data())
                .onItem().transform(result -> Response.ok(result).build())
                .onFailure().recoverWithItem(throwable -> {
