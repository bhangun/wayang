...
import tech.kayys.wayang.agent.type.OrchestratorAgent;
 ...
 */
@Executor(executorType = "orchestrator-agent", communicationType = CommunicationType.GRPC, maxConcurrentTasks = 3, supportedNodeTypes = {
        "agent-task", "orchestrator-agent-task", "coordination-task" })
@ApplicationScoped
public class OrchestratorAgentExecutor extends AbstractAgentExecutor {
    ...
    protected Uni<NodeExecutionResult> doExecute(NodeExecutionTask task) {
        ...
        String orchestrationTypeName = (String) context.getOrDefault("orchestrationType", "COLLABORATIVE");
        OrchestrationType orchestrationType = OrchestrationType.valueOf(orchestrationTypeName);

        String coordinationStrategyName = (String) context.getOrDefault("coordinationStrategy", "CENTRALIZED");
        CoordinationStrategy coordinationStrategy = CoordinationStrategy.valueOf(coordinationStrategyName);

        List<Map<String, Object>> agentTasks = (List<Map<String, Object>>) context.getOrDefault("agentTasks",
                List.of());
    ...
    private Uni<Map<String, Object>> executeAgentTask(
        ...
        NodeExecutionTask subTask = new NodeExecutionTask(
                parentTask.runId(),
                new tech.kayys.gamelan.engine.node.NodeId(parentTask.nodeId().value() + "-" + agentType),
                parentTask.attempt(),
                parentTask.token(),
                agentContext,
