...
public class AgentExecutor implements WorkflowExecutor {
                    ...
                    if (tool == null) {
                        return Uni.createFrom().item(
                                ToolResult.error(
                                        ...
                                        Map.of("errorCode", ErrorCode.TOOL_NOT_FOUND.getCode(),
                                                "retryable", ErrorCode.TOOL_NOT_FOUND.isRetryable())));
                    }
                            ...
                            .flatMap(isValid -> {
                                    ...
                                    return Uni.createFrom().item(
                                            ToolResult.error(
                                                    ...
                                                    Map.of("errorCode", ErrorCode.VALIDATION_FAILED.getCode(),
                                                            "retryable", ErrorCode.VALIDATION_FAILED.isRetryable())));
                                }
                                ...
                                return tool.execute(toolCall.arguments(), context)
                                        ...
                                        .onFailure().recoverWithItem(error -> ToolResult.error(
                                                ...
                                                toolCall.name(),
                                                error.getMessage(),
                                                Map.of("errorCode", ErrorCode.TOOL_EXECUTION_FAILED.getCode(),
                                                        "retryable", ErrorCode.TOOL_EXECUTION_FAILED.isRetryable())));
                            });
                })
                .onItem().invoke(result -> {
    ...
    protected NodeExecutionResult createFailureResult(
        ...
        return new DefaultNodeExecutionResult(
                ...
                java.util.Collections.emptyMap(),
                new ErrorInfo(
                        ErrorCode.AGENT_PLAN_FAILED.getCode(),
                        error.getMessage() != null ? error.getMessage() : "Unknown error",
                        getStackTrace(error),
                        Map.of("retryable", ErrorCode.AGENT_PLAN_FAILED.isRetryable())),
                task.token());
    }
