...
/**
 * Long-term memory executor implementing vector-based persistent memory storage.
...
@ApplicationScoped
@Executor(
    executorType = "longterm-memory-executor",
    communicationType = CommunicationType.GRPC,
    maxConcurrentTasks = 30,
    supportedNodeTypes = {"longterm-memory", "longterm-memory-task", "vector-memory", "persistent-memory"},
    version = "1.0.0"
)
public class LongTermMemoryExecutor extends AbstractMemoryExecutor {
    ...
    private Uni<Map<String, Integer>> processDeletesAndUpdates(List<String> toDelete, List<Memory> toUpdate) {
        ...
        return deleteCount.flatMap(deleted -> 
            updateCount.map(updated -> Map.of("deleted", deleted, "updated", updated))
        );
    }
