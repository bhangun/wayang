...
/**
 * Semantic memory executor for storing and retrieving factual knowledge and concepts.
 * Optimized for general knowledge, definitions, relationships, and conceptual understanding.
 * Supports knowledge graph-like queries and concept linking.
 */
@ApplicationScoped
@Executor(
    executorType = "semantic-memory-executor",
    communicationType = CommunicationType.GRPC,
    maxConcurrentTasks = 30,
    supportedNodeTypes = {"semantic-memory", "semantic-memory-task", "knowledge-memory", "concept-memory"},
    version = "1.0.0"
)
                ...
                .onItem().transform(scoredMemories -> {
                    ...
                    Map<String, List<Map<String, Object>>> groupedByCategory = entries.stream()
                            .collect(Collectors.groupingBy(e -> 
                                (String) ((Map<String, Object>) e.get("metadata")).getOrDefault("category", defaultCategory)));
