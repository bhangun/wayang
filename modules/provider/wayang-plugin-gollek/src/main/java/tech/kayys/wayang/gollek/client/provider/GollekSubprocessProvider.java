...
import tech.kayys.wayang.inference.InferenceRequest;
import tech.kayys.wayang.inference.InferenceResponse;
import tech.kayys.wayang.inference.StreamingInferenceChunk;
 ...
 * ({@code META-INF/services/tech.kayys.wayang.provider.LLMProvider}), so it
...
public class GollekSubprocessProvider implements StreamingProvider {
    ...
    private static String extractPrompt(ProviderRequest request) {
        ...
        for (tech.kayys.wayang.Message msg : request.getMessages()) {
            if (msg.getRole() == tech.kayys.wayang.Message.Role.SYSTEM && msg.getContent() != null) {
        ...
        for (tech.kayys.wayang.Message msg : request.getMessages()) {
            if (msg.getContent() != null) {
                if (msg.getRole() == tech.kayys.wayang.Message.Role.USER) {
                    sb.append("User: ").append(msg.getContent()).append("\n");
                } else if (msg.getRole() == tech.kayys.wayang.Message.Role.ASSISTANT) {
