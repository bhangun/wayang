 ...
 */
public final class DefaultWayangAgent implements Agent {
    ...
    private void runUntilDone(WayangAgentListener listener) {
        ...
        for (int iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
            ...
            for (ContentBlock.ToolUse call : acc.toolUses) {
                ...
                if (!autoApproveTools && !sessionApprovedTools.contains(tool.getId())) {
    ...
    private List<ToolSpec> buildToolSpecs() {
        ...
        for (Tool t : toolIndex.values()) {
            specs.add(new ToolSpec(t.getId(), t.descriptor().description(), null));
        }
        return specs;
    }
    ...
    private static Map<String, Tool> indexTools(List<Tool> tools) {
        ...
        for (Tool t : tools) index.put(t.descriptor().name(), t);
    ...
    // ── Tool SPI bridge helpers ───────────────────────────────────────────────

    /** Minimal ToolInvocation that adapts a Map<String,Object> from the provider. */
    private record SimpleToolInvocation(String name, Map<String, Object> arguments)
            implements ToolInvocation {
        @Override public tech.kayys.wayang.identity.ResourceId id() {
            return new tech.kayys.wayang.identity.ResourceId.ToolId(
                    tech.kayys.wayang.extension.Id.fromString(
        ...
        @Override public tech.kayys.wayang.extension.Metadata metadata() { return null; }
    }

    /** Minimal ToolContext backed by a working-directory attribute. */
    private static final class SimpleToolContext implements ToolContext {
        private final Map<String, Object> attrs;
        SimpleToolContext(java.nio.file.Path workspace) {
            attrs = workspace != null
                    ? Map.of("workspace", workspace.toString())
                    : Map.of();
        }
        @Override public Map<String, Object> attributes() { return attrs; }
        @Override public java.util.Optional<Object> getAttribute(String key) {
            return java.util.Optional.ofNullable(attrs.get(key));
        }
    }
}
