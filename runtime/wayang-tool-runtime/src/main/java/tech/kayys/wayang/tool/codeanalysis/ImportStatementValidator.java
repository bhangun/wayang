 ...
 * Detects broken imports where the imported class doesn't exist in the
 * specified package.
 * 
 * <p>
 * This validator helps catch issues like:
 * <ul>
 * <li>Typos in import statements</li>
 * <li>Imports from wrong packages (e.g., tech.kayys.alkhawarizm when should be
 * tech.kayys.gollek)</li>
...
public class ImportStatementValidator {
    ...
    private static final Pattern IMPORT_PATTERN = Pattern
            .compile("(?m)^import\\s+((?:[a-zA-Z_][a-zA-Z0-9_]*\\.)+[a-zA-Z_][a-zA-Z0-9_]*)\\s*;");

    private static final Pattern PACKAGE_PATTERN = Pattern
            .compile("(?m)^package\\s+((?:[a-zA-Z_][a-zA-Z0-9_]*\\.)+[a-zA-Z_][a-zA-Z0-9_]*)\\s*;");
    ...
    private String findSimilarPackage(String fullClassName) {
        ...
        // e.g., tech.kayys.alkhawarizm -> tech.kayys.gollek
        String[] commonReplacements = {
                "tech.kayys.alkhawarizm", "tech.kayys.gollek",
                "tech.kayys.gollek", "tech.kayys.alkhawarizm"
        };

        for (String prefix : commonReplacements) {
            if (wrongPackage.startsWith(prefix)) {
                String replacement = wrongPackage.replace(prefix,
                        prefix.equals("tech.kayys.alkhawarizm") ? "tech.kayys.gollek" : "tech.kayys.alkhawarizm");
                String candidate = replacement + "." + className;
                if (classToPackageCache.containsKey(candidate) || packageToClassesCache
                        .getOrDefault(replacement, Collections.emptySet()).contains(className)) {
                    return candidate;
                }
            }
        }

        // Fallback: search filesystem for files matching the class name and extract
        // package
        try {
            try (Stream<Path> paths = Files.walk(codebaseRoot)) {
