...
- Imports from wrong packages (e.g., `tech.kayys.alkhawarizm` when should be `tech.kayys.gollek`)
- Imports of non-existent classes
- Package/class name case sensitivity issues

## Problem Solved

This feature was created in response to a real issue where the Gollek project had many Java files importing `tech.kayys.alkhawarizm.tokenizer.spi.Tokenizer`, but the `Tokenizer` interface actually existed in `tech.kayys.gollek.tokenizer.spi.Tokenizer`. This caused compilation failures until all the imports were manually fixed.
...
- Handles common package prefix replacements (e.g., `tech.kayys.alkhawarizm` ↔ `tech.kayys.gollek`)
